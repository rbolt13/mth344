...
The $\underline{\text{range}}$ of $f:A\rightarrow B$ si the set $$ran(f)=\{f(a)|a\in A\}\subseteq B.$$
...
We can also decsribe this function as follows : $$f=(\begin{smallmatrix} 1 & 2 & 3\\ a & b&c\end{smallmatrix})$$
...
2. $f:A\rightarrow B$ is $\underline{\text{surjective}}$ or $\underline{\text{onto}}$ fi for all $y\in B$ there exists $x\in A$ such that $$f(x)=y.$$

$\underline{\text{Ex:}}f=(\begin{smallmatrix} 1 & 2 & 3\\ a & b&c\end{smallmatrix})$ from $A=\{1,2,3\}$ to $B=\{a,b,c\}$ is neither one-to-one nor onto (since there is a repetition in the second row and $c\in B$ doesn't appear in the second row)
...
$\underline{\text{Comment:}}$ If $f:A\rightarrow B$ has an inverse, then for all $x\in A$ and $y\in B$, $(f^{-1}\circ f)(x)=x$ and $(f\circ f^{-1})(y)=y$. 
