...
Let m,n $\in\mathbb{Z}$ and $n>0$. Then there exists q,r $\in \mathbb{Z}$ such that $$\frac{m}{n}=q+\frac{r}{n}\quad\quad\text{and}\quad\quad0\leq\frac{r}{n}<1$$
...
Let G be a group nd let $a\in G$ have order $n<\infty$. Then every power of a is equal to exactly one $$e,a,a^2,a^3,...,a^{n-1}.$$

$\underline{\text{Proof}}$:

Lets see why $e,a,a^2,a^3,...,a^{n-1}$ are distinct elements. 

Suppose that $a^i=a^i$ for some $0\leq i\leq j\leq n-1.$
