...
Since f is onto, there exists $x\in G$, such that $f(x)=y$. 
...
$\underline{\text{Comment}}$: Supppose every element of G squared equals the identity. This is equivalent to saying that every nonidentity element of G has order 2. 
...
The order of a cycle in $S_n$ is equal to tits length. So, $$ord(g)=6.$$
...
We need to add three 4's to get 0 in $\mathbb{Z}_6$. Similarly we need to add three 8's to get 0 in $\mathbb{Z}_{12}$.
